...
async function getDetails(req, res) {
    try {
        ...
        const recordDetail = await CategorySchema.findOne({_id:ObjectID(_id)});
...
async function getList(req, res) {
    ...
    try {
        ...
        if (page > 0) {
            ...
            let allRecords = await CategorySchema.aggregate([                
                {
                   $facet:{
                      data:[
           ...
           res.status(200).json(await Response.pagination(allRecords, responseMessage(reqObj.langCode,'SUCCESS'),pageData,req));
            ...
            let allRecords = await CategorySchema.find({}).lean();
